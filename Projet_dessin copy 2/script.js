...
document.getElementById('save-svg').addEventListener('click', () => {
    ...
    // Crée un SVG string avec un fond blanc
    let svgContent = `
        <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
            <rect width="100%" height="100%" fill="white" />
    `;

    // Convertit les données de dessin en SVG
    const imageData = ctx.getImageData(0, 0, width, height);
    const svgPath = generateSvgPathFromCanvas(imageData);
    svgContent += `<path d="${svgPath}" stroke="${brushColor}" fill="none" stroke-width="${brushSize}" />`;

    // Ferme la balise SVG
    svgContent += `</svg>`;

    // Crée un lien de téléchargement pour le fichier SVG
    const link = document.createElement('a');
